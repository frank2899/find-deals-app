...
const Results = () => {
    ...
    return (
        <Container maxW="container.lg" mt="5" p="0">
            {isErrorFetching && !isLoading ? (
                <Text textAlign="center" p="1rem" fontSize=".9rem">
                    Unable to find good deals based on your search.
                </Text>
            ) : (
                <></>
            )}
                ...
                <>
                    {location ? (
                        <Flex alignItems="center" mb=".5rem">
                            <BiCurrentLocation />
                            <Text color="gray" fontSize=".8rem" ml="5px">
                                {location}
                            </Text>
                        </Flex>
                    ) : (
                    ...
                    <SimpleGrid columns={[1, 2, 3]} spacing={5}>
                        {results.map((e: ProductResponse) => (
                            <ProductCard details={e} />
                        ))}
                    </SimpleGrid>
                </>
            ) : (
